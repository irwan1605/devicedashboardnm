...
export default function DeviceDashboard() {
  ...
  const offices = [
    "Polda Metro Jaya", "Polda Jabar", "Polda Jatim", "Polda Jateng", "Polda DIY",
    "Polda Sumut", "Polda Sumbar", "Polda Riau", "Polda Sumsel", "Polda Aceh",
    "Polda Kalbar", "Polda Kalsel", "Polda Kaltim", "Polda Kaltara",
    "Polda Sulut", "Polda Sulteng", "Polda Sulsel", "Polda Sultra",
    "Polda Bali", "Polda NTT", "Polda Maluku", "Polda Malut", "Polda Papua"
  ];
  ...
  const MapUpdater = ({ region }) => {
    ...
    useEffect(() => {
      if (region) {
        ...
        if (match) {
          map.flyTo(match.coords, 7, { duration: 1.5 });
          setBlinkCity(match.name);
          const t = setTimeout(() => setBlinkCity(""), 2500);
            ...
            <Card className="shadow-lg border h-full flex flex-col">
              ...
              <CardContent className="flex-1 h-[640px] p-0 relative">
                <MapContainer
                  center={[-2, 118]}
                  zoom={5}
                  className="h-full w-full rounded-b-lg transition-all duration-700"
                >
                  ...
                  {regionData
                    .filter((r) =>
                      !filters.region ||
                      r.name.toLowerCase().includes(filters.region.toLowerCase())
                    )
                    .map((r) => (
                      <Marker
                        key={r.name}
                        position={r.coords}
                        icon={createColoredIcon(islandColors[r.island])}
                        opacity={blinkCity === r.name ? 1 : 0.85}
                      >
                        <Popup>
                          <b>{r.name}</b> <br />
                          Pulau: {r.island}
                ...
                <TableBody>
                  {filteredDevices.map((d) => (
                    <TableRow key={d.id}>
                      ...
                      <TableCell
                        className={`font-bold ${
                          d.active === "Yes" ? "text-green-600" : "text-red-600"
                        }`}
                      >
                        {d.active}
                      </TableCell>
              ...
              {["region", "office", "status"].map((field) => (
                <div key={field} className="relative">
                  ...
                  <input
                    ...
                    onFocus={() =>
                      setDropdownVisible((p) => ({ ...p, [field]: true }))
                    }
